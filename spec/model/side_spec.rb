...
describe Moo::Model::Side do
  describe "type=" do
    it "should complain if side type is not 'image' or 'details'" do
      side = Side.new
      expect { side.type = 'foo' }.to raise_error(ArgumentError, "type must be either 'image' or 'details'")
    end

    it "should not complain if side type is 'image' or 'details'" do
      side = Side.new
      expect { side.type = 'image' }.to_not raise_error(ArgumentError)
      expect { side.type = 'details' }.to_not raise_error(ArgumentError)
    end
  end
  ...
  describe "pack=" do
    it "should complain if pack is not a pack object" do
      side = Side.new
      expect { side.pack = Hash.new }.to raise_error(ArgumentError, "pack must be of type Pack, Hash given")
    end
  end
  ...
  describe "template_code=" do
    it "should complain on an invalid template code" do
      side = Side.new
      expect { side.template_code = 'slartibartfast' }.to raise_error(ArgumentError, "invalid templatecode")
    end

    it "should not complain on valid template code" do
      side = Side.new
      expect { side.template_code = 'businesscard_full_image_landscape' }.to_not(
